...
import services.DB
from services.config import BOSS_BG_URLS, boss_names
from services.components import draw_ranking_card, get_paginated_data, draw_pagination_buttons, draw_search_bar
...
def rank_page_display(): 
    ...
    reset_keys = [f"page_{boss}" for boss in boss_names]  # 검색어가 바뀌면 페이지도 1로 초기화하기 위한 키 리스트
            ...
            for rank_idx, item in enumerate(paginated_data):
                ...
                with btn_cols[1]:
                    if st.button("상세 보기", key=f"btn_{boss_name}_{item['nickname']}_{real_rank}", width='stretch'):
                        ...
                        st.session_state.previous_page = "pages/rank.py"  # 이전 페이지 정보도 저장
